...
const refreshAccessToken = asyncHandler(async (req, res) => {
  const refreshIncomingRefreshToken =
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
  ...
  if (!channel?.length) {
    throw new ApiError(404, "Channel Does not E");
  }

  return res
    .status(200)
    .json(
      new ApiResponse(200, channel[0], "User Details Feteched Successfully"),
    );
});
...
const getWatchHistory = asyncHandler(async (req, res) => {
  const user = await User.aggregate([
    {
      $match: new mongoose.Schema.ObjectId(req.user._id),
    },
    {
      $lookup: {
        from: "videos",
